 ...
 * <p>
 * This class provides CRUD functionality for all Commentrating entities. It
...
@Stateful
@ConversationScoped
public class FrontCommentratingBean implements Serializable {
	...
	public Commentratingtype findCommentratingtypeById(Integer id) {
	...
	public String update() {
		...
		try {
			if (this.id == null) {
				String strMsg = tryInsertComment();
				simpleLoginBean.setMessageCommentRating(strMsg);
				if (strMsg.compareTo(Constants.COMMENT_RATING_SUCCESS) == 0) {
					simpleLoginBean.setIntentCountCommentRating(0);
				...
				return "view?faces-redirect=true&id="
						+ this.commentrating.getIdComment();
			}
		} catch (Exception e) {
	...
	private String tryInsertComment() {
		String strStatus = "";
		if (this.commentrating.getCachuelo().getCachuelostate()
				.getIdCachueloState() == 4
				|| this.commentrating.getCachuelo().getCachuelostate()
						.getIdCachueloState() == 3) {
			this.commentrating.setUser(userLogged);
			Commentratingtype comType = findCommentratingtypeById(1);
			this.commentrating.setCommentratingtype(comType);
			this.entityManager.persist(this.commentrating);
			strStatus = Constants.COMMENT_RATING_SUCCESS;
		} else {
			strStatus = Constants.COMMENT_RATING_FAILED;
		}
		return strStatus;
	}
	...
	public String updateFixed() {
		...
		try {
			if (this.id == null) {
				...
				Commentratingtype comType = findCommentratingtypeById(1);
				this.commentrating.setCommentratingtype(comType);
				this.commentrating.setCachuelo(simpleLoginBean
						.getCachueloToComment());
				this.entityManager.persist(this.commentrating);
				return "search?faces-redirect=true";
			} else {
				this.entityManager.merge(this.commentrating);
	...
	public String delete() {
		...
		try {
			...
			Commentratingtype commentratingtype = deletableEntity
					.getCommentratingtype();
	...
	public void paginate() {
		...
		CriteriaQuery<Commentrating> criteria = builder
				.createQuery(Commentrating.class);
		root = criteria.from(Commentrating.class);
		TypedQuery<Commentrating> query = this.entityManager
	...
	private Predicate[] getSearchPredicates(Root<Commentrating> root) {
		...
		Commentratingtype commentratingtype = findCommentRatingTypeById(1);
		if (commentratingtype != null) {
			predicatesList.add(builder.equal(root.get("commentratingtype"),
					commentratingtype));
		}
	...
	/*
	 * Support listing and POSTing back Commentrating entities (e.g. from inside
	...
	public Converter getConverter() {
		...
		final FrontCommentratingBean ejbProxy = this.sessionContext
				.getBusinessObject(FrontCommentratingBean.class);
